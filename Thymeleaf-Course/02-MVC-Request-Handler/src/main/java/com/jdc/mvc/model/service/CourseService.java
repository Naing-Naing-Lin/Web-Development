...
public class CourseService {
	...
	public int create(Course course) {
		...
		map.put("level", course.getLevel().name());
		map.put("price", course.getPrice());
		map.put("startDate", course.getStartDate());
		
		if(course.getId() > 0) {
			return insert.getJdbcTemplate().update("update course set name=?, price=?, duration=?, level=?, startDate=? where id=?", 
					course.getName(),
					course.getPrice(),
					course.getDuration(),
	...
	public Course findById(int id) {
		...
		return insert.getJdbcTemplate().queryForObject("select * from course where id = ?", rowMapper, id);
	}
